    ...
    public function handle_file_training_v2() {
        ...
        if (trim($data['contents']) !== '[]' && trim($data['contents']) !== '' && trim($data['contents']) !== 'null' && trim($data['contents']) !== NULL) {
            ...
            $id = $this->training_model->save_entry(
                    $data['contents'], $class, $data2['tokens'], $data2['counted'], $data2['removed_stop_words'], $data2['corpus_count'], $data2['meta_data'], $data2['bigram_raw'], $data2['bigram_counted'], $data2['final_tokens'], $data2['all_text'], $data2['toc']
            );
